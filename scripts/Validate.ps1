...
# Stage 1: Controlled Candidate Acquisition.
...
$repositoryComponentReports = @()
$powerShellExecutableName = if ($script:IsWindowsHost) { 'pwsh.exe' } else { 'pwsh' }
$powerShellPath = Join-Path $PSHOME $powerShellExecutableName
if (-not (Test-Path -LiteralPath $powerShellPath -PathType Leaf)) {
    throw "Canonical repository-test PowerShell executable is missing: $powerShellPath"
}
Assert-NoReparseAncestors -Path $powerShellPath -Context 'Canonical repository-test PowerShell executable'
...
foreach ($skillId in $skillIds) {
    ...
    $expectedInventoryPaths = @(
        @($integrityReport.skills | Where-Object { $_.skillId -ceq $skillId })[0].files |
...
foreach ($componentRelativePath in $repositoryComponents) {
    $componentPath = Join-Path $repoRoot $componentRelativePath.Replace('/', [IO.Path]::DirectorySeparatorChar)
    if (-not (Test-Path -LiteralPath $componentPath -PathType Leaf)) {
        throw "Canonical Repository Tests component is missing: $componentRelativePath"
    }
    Assert-NoReparseAncestors -Path $componentPath -Context "Canonical Repository Tests component '$componentRelativePath'"
    $componentOutput = Invoke-NativeChecked `
        -Command $powerShellPath `
        -Arguments @('-NoLogo', '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', $componentPath, '-RepositoryRoot', $repoRoot) `
        -Context "Repository Tests component '$componentRelativePath'" `
        -DiagnosticRoot $runRoot `
        -ChildWritableRoot $childOutputRoot `
        -IsolateRunnerCommandFiles `
        -TerminateProcessTree `
        -ProtectRunnerCommandFiles
    $componentOutputPath = Join-Path $runRoot ("repository-component-{0}.txt" -f ([IO.Path]::GetFileNameWithoutExtension($componentRelativePath)))
    [IO.File]::WriteAllText($componentOutputPath, ($componentOutput -join [Environment]::NewLine) + [Environment]::NewLine, [Text.UTF8Encoding]::new($false))
    $componentReceiptPath = Join-Path $runRoot ("repository-component-{0}.json" -f ([IO.Path]::GetFileNameWithoutExtension($componentRelativePath)))
    $componentReceipt = [ordered]@{
        component = $componentRelativePath
        result = 'passed'
        exitCode = 0
        output = [IO.Path]::GetFileName($componentOutputPath)
    }
    [IO.File]::WriteAllText($componentReceiptPath, ($componentReceipt | ConvertTo-Json -Depth 10) + [Environment]::NewLine, [Text.UTF8Encoding]::new($false))
    $repositoryComponentReports += [pscustomobject][ordered]@{
        component = $componentRelativePath
        result = 'passed'
        receipt = [IO.Path]::GetFileName($componentReceiptPath)
    }
}
...
$trustedPesterCommit = if ($isGitHubActions) {
    if (-not [string]::IsNullOrWhiteSpace($declaredTrustedSupervisorCommit)) {
        $declaredTrustedSupervisorCommit
    }
    else {
...
$summary = [pscustomobject][ordered]@{
    ...
    packageInventory = @($integrityReport.skills)
    ...
    toolReceipts = @($expectedSources.Keys | ForEach-Object {
        [pscustomobject][ordered]@{
            toolName = $_
            receipt = "receipt-$_.json"
            source = [string]$receipts[$_].source
            version = [string]$receipts[$_].resolvedVersion
            resolvedIdentity = [string]$receipts[$_].resolvedIdentity
        }
    ...
    stages = [ordered]@{
        ...
        skillspectorStatic = $staticReports
           repositoryTests = [ordered]@{
               ...
               components = $repositoryComponentReports
    ...
    stageResults = [ordered]@{
        ...
        skillspectorStatic = 'passed'
        repositoryTests = 'passed'
        conditionalSemanticScan = if ($semanticTriggered) { if (@($securityBlockers).Count -eq 0) { 'passed' } else { 'blocked' } } else { 'not-run' }
    }
    deviations = 'None'
    result = if (@($securityBlockers).Count -eq 0) { 'passed' } else { 'blocked' }
}
