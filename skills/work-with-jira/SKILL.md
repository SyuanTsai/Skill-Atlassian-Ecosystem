...
1. In IDE GitHub Copilot, use this same Skill; do not require an Atlassian connector when the user selected the verified REST path. Before the first REST query, resolve the installed `configure-jira-api-access` Skill and its `scripts/Test-JiraApiAccess.ps1` relative to the installed Skill directory. Do not hardcode repository, `.agents`, `.github`, or home-directory paths.
2. If that validator reports `reload-required` or `process-user-mismatch`, apply `configure-jira-api-access/references/copilot-ide.md` and the shared `HostReloadContract`, recreate the IDE/Copilot host when required, then rerun the same validator. Do not diagnose an inheritance mismatch as a bad token and do not create a Copilot-only access implementation.
