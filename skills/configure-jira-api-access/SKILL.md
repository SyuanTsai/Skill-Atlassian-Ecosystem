...
6. Before token creation or rotation, show the minimum read-only scope checklist for the intended Jira operation. For the validator's identity plus issue and JQL reads, prefer classic `read:jira-user` and `read:jira-work`. If granular scopes are required, combine the documented identity scopes with the exact scopes for each selected GET endpoint; do not reuse POST-search scopes or assume the enhanced-JQL set covers a different endpoint. Do not request write scopes for read-only validation.
...
8. To preflight the requested read endpoint, run the validator with a confirmed `-IssueKey`, a confirmed `-Jql`, or both. Keep `-MaxResults` at or below 100. These modes perform GET requests only and suppress response bodies. This readiness check does not replace the user-visible read through `work-with-jira`.
9. Classify failures safely: `400` request or configuration, `401` authentication, `403` authorization or scope, `404` endpoint or resource, `429` rate-limit, `5xx` service unavailable, and transport, network, or TLS separately.
...
- `401`: verify token type, account email, expiration or revocation, and scoped-token endpoint selection.
...
- Network or TLS failure: separate proxy, DNS, certificate, or network policy failures from authentication failure.
...
- the identity, issue, and JQL endpoint categories tested and HTTP status;
